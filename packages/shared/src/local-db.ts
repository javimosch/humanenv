...
export interface ILocalDatabase {
  ...
  getProjectById(id: string): Promise<{ id: string; name: string; createdAt: number; fingerprintVerification: boolean; requireApiKey: boolean } | null>
...
export class LocalDatabase implements ILocalDatabase {
  ...
  async getProjectById(id: string): Promise<{ id: string; name: string; createdAt: number; fingerprintVerification: boolean; requireApiKey: boolean } | null> {
    const row = this.db.prepare('SELECT id, name, fingerprint_verification, require_api_key, created_at FROM projects WHERE id = ?').get(id) as any
    return row ? { id: row.id, name: row.name, createdAt: row.created_at, fingerprintVerification: !!row.fingerprint_verification, requireApiKey: !!row.require_api_key } : null
  }
...
export function createLocalDatabase(dbPath: string): { db: LocalDatabase } {
