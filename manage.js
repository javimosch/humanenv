...
async function selectEnvFile() {
  ...
  const envFiles = files.filter(
    (f) => f.startsWith(".env") && f !== ".env.example" && !f.endsWith(".local"),
  );
...
async function main() {
  ...
  const config = {
    ...
    REMOTE_SERVICE_NAME: process.env.REMOTE_SERVICE_NAME || "humanenv-server",
  ...
  async function followLogs() {
    ...
    const sshCmd = `ssh -t -p ${config.REMOTE_PORT} ${config.REMOTE_USER}@${config.REMOTE_HOST} "cd ${config.REMOTE_PATH} && docker compose -f ${composeFile} logs -f ${config.REMOTE_SERVICE_NAME}"`;
